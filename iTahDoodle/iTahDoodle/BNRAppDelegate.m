...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions
{
    ...
    if ([tasks count] == 0)
    {
        ...
        [tasks addObject:@"Note 1: Hello :D"];
        [tasks addObject:@"Note 2: Weeee"];
        [tasks addObject:@"Chop the logs"];
    }
    
    
    //1. Create and configure the UIWindow instance
    //A CGRect is a struct with an origin (x,y) and size (width, height)
    CGRect windowFrame = [[UIScreen mainScreen] bounds];
    UIWindow *theWindow = [[UIWindow alloc] initWithFrame:windowFrame];
    [self setWindow:theWindow];
    
    //2. Define the frame rectangles of the three UI elements
    //CGRectMake() creates a CGRect from (x, y, width, height)
    CGRect tableFrame = CGRectMake(0,80,320,380);
    CGRect fieldFrame = CGRectMake(20, 40, 200, 31);
    CGRect buttonFrame = CGRectMake(228, 40, 72, 31);
    
    //3. Create and configure the table view
    ...
    //4. Create and configure the text field where new tasks will be typed.
    taskField = [[UITextField alloc] initWithFrame:fieldFrame];
    [taskField setBorderStyle:UITextBorderStyleRoundedRect];
    [taskField setPlaceholder:@"Type a task! :D"];
    
    //5. Create and configure a rounded rect Insert button
    insertButton = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [insertButton setFrame:buttonFrame];
    
    //Buttons behave using a target/action callback
    //6. Configure the Insert button's action to call this object's -addTask: method
    [insertButton addTarget:self action:@selector(addTask:) forControlEvents:UIControlEventTouchUpInside];
    
    //7. Give the button a title
    [insertButton setTitle:@"Done" forState:UIControlStateNormal];
    
    //8. Add our three UI elements to the window
    //Looks like it's gotta be done to add these UIclasses to the "view" (the screen)
    [[self window] addSubview:taskTable];
    [[self window] addSubview:taskField];
    [[self window] addSubview:insertButton];
    
    //9. Finalize the window and put it on the screen
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    /*To improve performance, we reconfigure cells in memory
     that have scrolled off the screen and hand them back with new
     contents instead of always creating new cells.
...
-(void) addTask:(id)sender
{
    ...
    [tasks addObject:t];
    //Refresh the table so that the new items show up
...
- (void)applicationWillTerminate:(UIApplication *)application
{
    ...
    //This method is for applications of iOS version < 4.0
    [tasks writeToFile:docPath() atomically:YES];
}
